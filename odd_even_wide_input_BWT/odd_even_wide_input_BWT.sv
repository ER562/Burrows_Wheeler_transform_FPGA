        ...
        if(rst)begin    //reset always have priority
            ...
            if(state == 3)begin //reading data only
                if(memory_pointer >= real_data_length)begin    //no more data to read. returns to state 0
                    ...
                    for(int i = 0 ; i < DATA_POINTS_PER_CYCLE ; i++)begin
                        if(memory_pointer + i < real_data_length)begin
                            read_data[i * DATA_WIDTH +: DATA_WIDTH] <= data_memory[move_memory[memory_pointer + i] == 0 ? real_data_length - 1 : move_memory[memory_pointer + i] - 1];
                            if(move_memory[memory_pointer + i] == 0)begin
                                original_line_idx = i;
                            end
                            read_data_points++;
                        end
                    ...
                    memory_pointer <= memory_pointer + read_data_points;
                end
                done <= 0;
            end
