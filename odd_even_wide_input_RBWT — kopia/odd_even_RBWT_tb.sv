    ...
    initial begin
        ...
        for(int z = 0 ; z < NUMBER_OF_TEST_WORDS ; z = z + 1)begin
            ...
            for(int i = 0 ; i < input_data_length[z] ; i = i + DATA_POINTS_PER_CYCLE)begin
                ...
                automatic int original_line_idx_temp = DATA_POINTS_PER_CYCLE;
                for(int j = 0 ; j < DATA_POINTS_PER_CYCLE ; j++)begin 
                    ...
                    if(i + j == original_line_idx[z])begin
                        original_line_idx_temp = j;
                    end
                end
                write_data_size <= chunk_size;
                original_line <= original_line_idx_temp;
                @(posedge clk);
            end
            write_en <= 0;
