...
module odd_even_BWT_tb #(
    ...
    int output_bytes [0 : NUMBER_OF_TEST_WORDS - 1][0 : INPUT_ARRAY_LENGTH - 1]; 
    string output_data;
    ...
    initial begin
        ...
        write_en <= 1;
        for(int i = 0 ; i < input_data_length[0] ; i = i + DATA_POINTS_PER_CYCLE)begin
            automatic int chunk_size = 0;
            if(done == 1)begin
                sorting_done = 1;
            end
            for(int j = 0 ; j < DATA_POINTS_PER_CYCLE ; j++)begin 
                if(i + j < input_data_length[0])begin
                    write_data[j * DATA_WIDTH +: DATA_WIDTH] <= input_data[0][i + j];
                    chunk_size++;
                end
            end
        ...
        //writing and reading rest words
        for(int z = 1 ; z < NUMBER_OF_TEST_WORDS ; z++)begin
            write_en <= 1;
            ...
            for(int i = 0 ; i < input_data_length[z - 1] ; i = i + DATA_POINTS_PER_CYCLE)begin
                if(done == 1)begin
                    sorting_done = 1;
                end
                for(int j = 0 ; j < DATA_POINTS_PER_CYCLE ; j++)begin
                    if(i + j < input_data_length[z - 1])begin
                        output_bytes[z - 1][i + j] = read_data[j * DATA_WIDTH +: DATA_WIDTH];
                        if(original_line != DATA_POINTS_PER_CYCLE)begin
                            original_line_idx[z - 1] <= i + original_line;
                        end
                    end
                end
                @(posedge clk);
        ...
        @(posedge clk);
        @(posedge clk);
        for(int i = 0 ; i < input_data_length[NUMBER_OF_TEST_WORDS - 1] ; i = i + DATA_POINTS_PER_CYCLE)begin
            for(int j = 0 ; j < DATA_POINTS_PER_CYCLE ; j++)begin
                if(i + j < input_data_length[NUMBER_OF_TEST_WORDS - 1])begin
                    output_bytes[NUMBER_OF_TEST_WORDS - 1][i + j] = read_data[j * DATA_WIDTH +: DATA_WIDTH];
                    if(original_line != DATA_POINTS_PER_CYCLE)begin
                        original_line_idx[NUMBER_OF_TEST_WORDS - 1] <= i + original_line;
                    end
                end
            end
            @(posedge clk);
        ...
        //presenting data
        for(int z = 0 ; z < NUMBER_OF_TEST_WORDS ; z++)begin
            output_data = "";
            for(int i = 0; i < input_data_length[z]; i++) begin
                output_data = {output_data, string'(output_bytes[z][i])};
            end
            $display("\n------------------------------------------------------");
            $display("Test: %0d   word: %0s", z, input_data[z]);
            $display("Algorithm time: %0t ps", end_time - start_time);
            ...
            $display("Original line idx: %0d   Expected original line idx: %0d", original_line_idx[z], expected_original_line_idx[z]);
            if(output_data == expected_data[z] && original_line_idx[z] == expected_original_line_idx[z])begin
                $display("Test PASSED");
                test_passed++;
            end else begin
                $display("Test FAILED");
